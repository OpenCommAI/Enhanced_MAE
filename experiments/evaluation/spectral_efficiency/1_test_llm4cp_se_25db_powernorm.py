...
# 改成你的 LLM4CP best.pt 路径
CKPT_PATH = fr"./checkpoints/spectral_efficiency/outputs_llm4cp_25dB_8to1_1/LLM4CP_train_8to1_2026-06-03_11-11-28/best.pt"

SAVE_CSV = fr"./result_0605_multi_SE/test_llm4cp_paper_style_SE_{SE_SNR_DB}dB_denorm_powernorm_results.csv"
...
def update_cfg_from_checkpoint(ckpt):
    """
    如果 checkpoint 里保存了 config，则优先使用训练时的模型参数。
    """
...
@torch.no_grad()
def evaluate_llm4cp_paper_style_se(
    model,
    test_loader,
    device,
    zscore_stats,
    ...
    pbar = tqdm(
        test_loader,
        desc=f"Testing LLM4CP denorm + powernorm paper-style SE at {SE_SNR_DB} dB",
        ncols=170,
    )
    ...
    for batch_idx, (x_input, y_true_norm) in enumerate(pbar):
        ...
        with autocast_ctx():
            model_loss, pred_norm, aux, loss_dict = model(xb, yb_norm)
...
def main():
    ...
    print("\n" + "=" * 100)
    print("LLM4CP 测试：25 dB Paper-style SE，反 Z-score + 信道功率归一化")
    ...
    result = evaluate_llm4cp_paper_style_se(
        model=model,
        test_loader=test_loader,
        device=device,
        zscore_stats=zscore_stats,
    ...
    print("\n" + "=" * 100)
    print("LLM4CP 25 dB 测试集 Paper-style SE 结果：反 Z-score + 信道功率归一化")
