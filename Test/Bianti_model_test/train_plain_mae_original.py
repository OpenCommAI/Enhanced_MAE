...
class Config:
    ...
    # 改这里：训练输出保存路径
    out_root: str = r"./result_new_0604_bianti_v/outputs_plain_mae_speed_test"
...
def make_train_test_loaders(Xtr, Ytr, Xte, Yte, use_cuda: bool):
    train_loader, train_size = make_loader(Xtr, Ytr, use_cuda=use_cuda, shuffle=True)
    test_loader, test_size = make_loader(Xte, Yte, use_cuda=use_cuda, shuffle=False)
...
def evaluate_speed_tests(model, device, run_dir: str, amp_enabled=True):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        fieldnames = ["speed_kmh", "samples", "loss_nmse", "nmse_db", "speed_dir"]
...
def main():
    ...
    run_name = f"plain_mae_speed_8to{cfg.pred_tti}_keep{cfg.keep_patches}_{timestamp()}"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        line = (
            ...
            f"total_test_nmse: {test_loss:.6e} ({test_nmse_db:.2f} dB), "
            f"lr: {lr_now:.6g}, "
            f"keep_patches: {cfg.keep_patches}"
        )
