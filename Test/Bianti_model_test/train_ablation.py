...
from models_mae_ablation import (
    build_mae_channel_hop_only,
    build_mae_channel_multiscale_only,
)
...
class Config:
    ...
    # 可选:
    #   "hop_only"         : 只有跳频采样
    #   "multiscale_only"  : 只有多尺度融合
    # ========================================================
    model_type: str = "multiscale_only"
    # model_type: str = "hop_only"
    ...
    #   all_speed_dataset/
    #   speed_test_dataset/
    data_root: str = r"/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_all_zhibiao/CSI_data_25dB/None_svd_speed_test_8to1_10to100_step2_v1/"

    all_dataset_name: str = "all_speed_dataset"
    speed_test_name: str = "speed_test_dataset"

    # 改这里：训练结果输出目录
    out_root: str = r"./result_new_0604_bianti_v/outputs_ablation_speed_test_multiscale_only"
    ...
    # 5. hop_only 专用参数
    # ========================================================
    hop_steps: tuple = (1, 3, 5, 7)
    balanced_hop: bool = True

    # 只对 hop_only 有效。
    # 测试时分别用 1、3、5、7 做 forward，然后对 NMSE 求平均。
...
# ============================================================
def build_model():
    if cfg.model_type == "hop_only":
        model = build_mae_channel_hop_only(
            history_tti=cfg.history_tti,
            pred_tti=cfg.pred_tti,
    ...
    if cfg.model_type == "multiscale_only":
        model = build_mae_channel_multiscale_only(
            history_tti=cfg.history_tti,
            pred_tti=cfg.pred_tti,
        )
        return model
    ...
    raise ValueError(
        ...
        f"Choose from ['hop_only', 'multiscale_only']."
    )
...
def evaluate(model, loader, device, amp_enabled=True):
    ...
    use_eval_all_hops = (
        cfg.model_type == "hop_only"
        and cfg.eval_all_hops
    )

    eval_hops = cfg.hop_steps if use_eval_all_hops else None
...
def load_speed_test_loader(speed: int, use_cuda: bool):
    ...
    test_inputs_path = os.path.join(speed_dir, "test_inputs.npy")
    test_labels_path = os.path.join(speed_dir, "test_labels.npy")

    if not os.path.exists(test_inputs_path):
        raise FileNotFoundError(f"找不到 {speed}kmh test_inputs.npy: {test_inputs_path}")

    if not os.path.exists(test_labels_path):
        raise FileNotFoundError(f"找不到 {speed}kmh test_labels.npy: {test_labels_path}")

    print("\n" + "-" * 100)
    print(f"加载 {speed} km/h 独立测试集")
    print(f"Speed test dir: {speed_dir}")

    X, Y = load_pair(test_inputs_path, test_labels_path)

    loader, size = make_loader(
        X,
        Y,
        use_cuda=use_cuda,
        shuffle=False,
    ...
    return loader, size, speed_dir


@torch.no_grad()
def evaluate_speed_tests(model, device, run_dir: str, amp_enabled=True):
    print("\n" + "=" * 100)
    print("开始测试 10 个速度的独立测试集")
    print("=" * 100)
    ...
    for speed in cfg.speed_list:
        loader, sample_num, speed_dir = load_speed_test_loader(
            speed=speed,
            use_cuda=use_cuda,
        )

        loss, nmse_db = evaluate(
            model=model,
            loader=loader,
            device=device,
        ...
        print(
            ...
            f"samples={sample_num:>5} | "
            f"loss={loss:.6e} | "
            f"NMSE(dB)={nmse_db:.2f}"
        )

    csv_path = os.path.join(run_dir, "speed_test_results.csv")
    ...
    print("\n10 个速度测试结果汇总:")
    print(f"{'Speed':>10} | {'Samples':>10} | {'Loss(NMSE)':>15} | {'NMSE(dB)':>10}")
    print("-" * 60)
    ...
    for item in results:
        print(
            f"{item['speed_kmh']:>10} | "
            f"{item['samples']:>10} | "
            f"{item['loss_nmse']:>15.6e} | "
            f"{item['nmse_db']:>10.2f}"
        )
...
def main():
    ...
    assert cfg.pred_tti == 1, "当前消融实验保持 pred_tti=1"
    assert cfg.epochs > cfg.warmup_epochs
    assert cfg.model_type in ["hop_only", "multiscale_only"]
    ...
    run_name = f"{cfg.model_type}_speed_8to{cfg.pred_tti}_{timestamp()}"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for x_input, y_future in pbar:
            ...
            if cfg.model_type == "hop_only" and "hop_steps" in loss_dict:
    ...
    print(f"Best total test NMSE loss: {best_test_loss:.6e}")
    print(f"Best total test NMSE dB: {best_test_nmse_db:.2f} dB")
    print(f"Best model path: {best_path}")

    # ========================================================
    # 加载 best.pt，测试 10 个速度
    # ========================================================
    print("\n加载 best.pt，用最佳模型测试 10 个速度。")
    ckpt = torch.load(best_path, map_location=device)
    model.load_state_dict(ckpt["model"])
    model.to(device)
    model.eval()

    speed_results = evaluate_speed_tests(
        model=model,
        device=device,
        run_dir=run_dir,
        amp_enabled=cfg.amp,
    ...
    with open(log_path, "a", encoding="utf-8") as f:
        f.write("\n10 speed test results:\n")
        for item in speed_results:
            f.write(
                f"speed={item['speed_kmh']} km/h, "
                f"samples={item['samples']}, "
                f"loss={item['loss_nmse']:.6e}, "
                f"nmse_db={item['nmse_db']:.2f} dB\n"
            )
