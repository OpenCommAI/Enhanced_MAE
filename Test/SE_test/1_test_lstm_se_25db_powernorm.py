...
class Config:
    ...
    # 测试数据目录
    data_dir: str = (
        fr"/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_SE_test/CSI_data_test_label/{num2}dB_zscore_norm_8to1_dataset"
    )
    ...
    # LSTM 参数，需要和训练时一致；如果 ckpt 里有 config，会自动覆盖
    hidden_size: int = 256
    num_layers: int = 2
...
# 改成你的 LSTM best.pt 路径
CKPT_PATH = "/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_SE_test/result_0605_multi_SE/outputs_lstm_20dB_8to1_1/8to1_2026-06-03_11-09-58/best.pt"

# 结果保存路径
SAVE_CSV = fr"./result_0605_multi_SE/test_lstm_paper_style_SE_{SE_SNR_DB}dB_denorm_powernorm_results.csv"
...
@torch.no_grad()
def evaluate_lstm_paper_style_se(
    model,
    test_loader,
    device,
    zscore_stats,
    ...
    pbar = tqdm(
        test_loader,
        desc=f"Testing LSTM denorm + powernorm paper-style SE at {SE_SNR_DB} dB",
        ncols=170,
    )
...
def main():
    ...
    print("\n" + "=" * 100)
    print("LSTM 测试：25 dB Paper-style SE，反 Z-score + 信道功率归一化")
    ...
    result = evaluate_lstm_paper_style_se(
        model=model,
        test_loader=test_loader,
        device=device,
        zscore_stats=zscore_stats,
    ...
    print("\n" + "=" * 100)
    print("LSTM 25 dB 测试集 Paper-style SE 结果：反 Z-score + 信道功率归一化")
