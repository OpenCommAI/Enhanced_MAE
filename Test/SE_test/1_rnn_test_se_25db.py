...
class Config:
    ...
    data_dir: str = fr"/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_SE_test/CSI_data_test_label/{num2}dB_zscore_norm_8to1_dataset"
...
# 改成你的 RNN best.pt 路径
CKPT_PATH = "/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_SE_test/result_0605_multi_SE/outputs_rnn_20dB_8to1_1/rnn_8to1_2026-06-03_11-09-51/best.pt"

# 结果保存路径
SAVE_CSV = fr"./result_0605_multi_SE/test_rnn_paper_style_SE_{SE_SNR_DB}dB_denorm_powernorm_results.csv"

# 改成你的 Z-score 参数目录
ZSCORE_STATS_DIR = "./Zscore_original/20dB_svd090_norm"
...
def update_cfg_from_checkpoint(ckpt):
    ...
    print(f"rnn_nonlinearity : {cfg.rnn_nonlinearity}")
    print("=" * 100)
...
@torch.no_grad()
def evaluate_rnn_paper_style_se(
    model,
    test_loader,
    device,
    zscore_stats,
    ...
    pbar = tqdm(
        test_loader,
        desc=f"Testing RNN denorm + powernorm paper-style SE at {SE_SNR_DB} dB",
        ncols=170,
    )
...
def main():
    ...
    print("\n" + "=" * 100)
    print("RNN 测试：25 dB Paper-style SE，反 Z-score + 信道功率归一化")
    ...
    result = evaluate_rnn_paper_style_se(
        model=model,
        test_loader=test_loader,
        device=device,
        zscore_stats=zscore_stats,
    ...
    print("\n" + "=" * 100)
    print("RNN 25 dB 测试集 Paper-style SE 结果：反 Z-score + 信道功率归一化")
