...
class Config:
    ...
    ckpt_path: str = r"/home/ubuntu/zq_mae/CSI_data_xtti2ytti_svdDone/multi_model_all_zhibiao/Pth_all_model/LSTM.pt"
    out_dir: str = r"./lstm_eval_outputs"
...
def main():
    ...
    hidden_size = ckpt_cfg.get("hidden_size", cfg.hidden_size)
    num_layers = ckpt_cfg.get("num_layers", cfg.num_layers)
    bidirectional = ckpt_cfg.get("bidirectional", cfg.bidirectional)
    ...
    if isinstance(ckpt, dict):
        for key in ["epoch", "best_test_nmse", "best_test_nmse_db", "test_nmse", "test_nmse_db", "lr", "final_test_nmse", "final_test_nmse_db"]:
            if key in ckpt:
    ...
    summary_path = os.path.join(cfg.out_dir, "lstm_eval_summary.txt")
